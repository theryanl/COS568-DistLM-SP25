...
def train(args, train_dataset, model, tokenizer):
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            step_losses.append((global_step, step_loss))
            ...
            # Store timing but skip the first iteration
            if first_iteration:
        ...
        if iteration_times:
            ...
            logger.info(f"Average time per iteration (excluding first): {avg_time:.6f} seconds")
            print(f"Average time per iteration (excluding first): {avg_time:.6f} seconds")
        
        evaluate(args, model, tokenizer)
        ##################################################

    # save epoch and step losses to file
    epoch_loss_file = os.path.join(args.output_dir, f"epoch_loss_node_{args.local_rank}.txt")
    step_loss_file = os.path.join(args.output_dir, f"step_loss_node_{args.local_rank}.txt")
    ...
    # 1. Epoch-level loss plot
    plt.figure(figsize=(10, 6))
    epochs, losses = zip(*epoch_losses)
    plt.plot(epochs, losses, 'o-', linewidth=2)
    plt.title(f'Training Loss by Epoch (Node {args.local_rank})')
    plt.xlabel('Epoch')
    plt.ylabel('Average Loss')
    plt.grid(True)
    epoch_loss_fig_path = os.path.join(args.output_dir, f"epoch_loss_node_{args.local_rank}.png")
    plt.savefig(epoch_loss_fig_path)
    print(f"Epoch loss figure saved to {epoch_loss_fig_path}")
    plt.close()
    
    # 2. Step-level loss plot
    plt.figure(figsize=(12, 6))
    steps, step_loss_values = zip(*step_losses)
    plt.plot(steps, step_loss_values, '-', linewidth=1.5, alpha=0.7)
    plt.title(f'Training Loss by Step (Node {args.local_rank})')
    plt.xlabel('Global Step')
    plt.ylabel('Loss')
    plt.grid(True)
    plt.tight_layout()
    step_loss_fig_path = os.path.join(args.output_dir, f"step_loss_node_{args.local_rank}.png")
